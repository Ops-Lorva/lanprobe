...
if [[ -d /run/systemd/system ]]; then
  ...
  echo "  Create an enrolment code in the hub (Fleet -> + on a site), then:"
  echo "    sudo -u lanprobe lanprobe-server --config-dir /var/lib/lanprobe \\"
  ...
  echo "Attach it first: /usr/bin/lanprobe-server --config-dir /var/lib/lanprobe enroll --hub <url> --code <code>"
fi
