//! Chiffrement au repos des secrets applicatifs, partagé par le serveur
//! headless (`app_config.json`) et le hub web (identifiants de notification).
//!
//! Ce qui est chiffré ici : les **secrets applicatifs** (jeton InfluxDB v2,
//! mot de passe InfluxDB v1, mot de passe SMTP, URL de webhook). Les **mots
//! de passe utilisateur** ne passent jamais par ce module : ils sont hashés
//! en argon2id dans `passwords.rs`, et un mot de passe déchiffrable serait
//! une faille, pas une fonctionnalité.
//!
...
pub fn load_or_create_key(config_dir: &Path) -> Result<SecretKey, String> {
    ...
    let SecretKey { bytes } = generate_key()?;
...
/// Tire une clé neuve sans rien écrire sur disque. Sert au hub, dont les
/// tests travaillent sur une base en mémoire et n'ont aucun volume où poser
/// un `secret.key`.
pub fn generate_key() -> Result<SecretKey, String> {
...
/// soit absente — prend ce verrou. Il n'est pas sous `cfg(test)` : les tests
/// de `lanprobe-server` en ont besoin, et un `cfg(test)` ne franchit pas la
/// frontière d'un crate.
pub static ENV_KEY_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
